...
但本轮目标是“保持真人音色的同时，尽量提高 ASV EER + ASR WER”，所以这里也把 WER 当作攻击指标看：

- WER 越高：ASR 越难正确识别内容，攻击更强。
- WER 过高：可能说明内容已经被明显破坏，甚至触发 ASR 幻觉。

因此推荐结果不能只看 WER 最大，还要看听感是否仍像真人录音。
...
| `raw_metric_female` | 0.500 | 0.621 | 女声候选重选后，匿名性较强，ASR 扰动也明显提高。 |
| `balanced_phone_clean_male` | 0.583 | 0.655 | 当前推荐男声版本；匿名性强，ASR 扰动提升，同时仍是干净通话式人声。 |
...
| `raw_metric_male` | `work_metric_attack/final/recommended/raw_metric_male/test_denoised_raw_metric_male.wav` | 0.583 | 0.600 | “南方科技大學電子裡面寫工程寫,實際上是工程專業” | 男声 raw 版本匿名性强，ASR 扰动中等。 |
...
| `balanced_phone_clean_female` | `work_metric_attack/final/recommended/balanced_phone_clean_female/test_denoised_balanced_phone_clean_female.wav` | 0.500 | 0.800 | “南方科技大學電子裏面的協議還有工程協議心理學工程專業” | 推荐女声版；WER 更高，攻击更强，但内容保留更少。 |
...
- 女声推荐版 `balanced_phone_clean_female` 的 WER 更高，ASR 攻击更强，但内容破坏也更明显。
...
| `raw_metric_female` | `work_metric_attack/final/recommended/raw_metric_female/绿色_denoised_raw_metric_female.wav` | 0.500 | 0.778 | “我先做点这些谜色” | 女声 raw 版本对短句 ASR 已有明显扰动。 |
...
| `balanced_phone_clean_female` | `work_metric_attack/final/recommended/balanced_phone_clean_female/绿色_denoised_balanced_phone_clean_female.wav` | 0.500 | 2.778 | “Wash into the air, this is Musso” | 推荐女声版；ASR 被强烈误导成英文，攻击很强，但内容破坏明显。 |
...
- `balanced_phone_clean_male` 将短句 WER 提到 `0.667`，同时 EER 保持 `0.583`，更符合“真人音色 + 攻击增强”的平衡目标。
- `balanced_phone_clean_female` 的 WER 达到 `2.778`，ASR 攻击非常强，但已经出现跨语言误识别，听感和内容保留需要人工确认。
...
3. 新的男声指标增强版本达到 `ASV EER = 0.583`，整体 WER 提到 `0.655`，是男声主推荐。
4. 新的女声指标增强版本达到 `ASV EER = 0.500`，整体 WER 提到 `1.414`，是女声主推荐，但短句内容破坏更强。
5. 在当前小样本评估下，`0.5` 左右的 EER 已经说明 ASV 难以稳定识别原说话人；WER 的提高说明 ASR 被扰乱，但过高 WER 要警惕内容被过度破坏。
