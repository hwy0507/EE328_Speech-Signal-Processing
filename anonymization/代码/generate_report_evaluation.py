...
DOCUMENT_ROOT = PROJECT_ROOT / "文档"
BENCHMARK_INPUT_DIR = DOCUMENT_ROOT / "benchmark_inputs"
DOCUMENT_AUDIO_DIR = DOCUMENT_ROOT / "音频"
DEFAULT_OUTPUT_DIR = DOCUMENT_ROOT / "report_evaluation"
...
SELECTION_FILES = {
    "female_leaning": BENCHMARK_INPUT_DIR / "work_smooth_verify/final/preferred_variants/female_leaning_selections.json",
    "male_leaning": BENCHMARK_INPUT_DIR / "work_smooth_verify/final/preferred_variants/male_leaning_selections.json",
    "balanced_phone_clean_female": PROJECT_ROOT
    / "文档/benchmark_inputs/work_metric_attack/final/recommended/balanced_phone_clean_female/balanced_phone_clean_female_selections.json",
    "balanced_phone_clean_male": PROJECT_ROOT
    / "文档/benchmark_inputs/work_metric_attack/final/recommended/balanced_phone_clean_male/balanced_phone_clean_male_selections.json",
    "ppg_tone_female": PROJECT_ROOT
    / "文档/benchmark_inputs/work_ppg_tone/final/recommended/ppg_tone_female/ppg_tone_female_selections.json",
    "ppg_tone_male": PROJECT_ROOT
    / "文档/benchmark_inputs/work_ppg_tone/final/recommended/ppg_tone_male/ppg_tone_male_selections.json",
}
...
def resolve_benchmark_output(raw_path: str) -> Path:
    path = Path(raw_path).expanduser()
    if path.exists():
        return path.resolve()
    parts = path.parts
    for marker in ("work_metric_attack", "work_ppg_tone", "work_smooth_verify"):
        if marker in parts:
            marker_index = parts.index(marker)
            archived = BENCHMARK_INPUT_DIR / Path(*parts[marker_index:])
            if archived.exists():
...
def read_selection_outputs(path: Path) -> list[Path]:
    ...
    return [resolve_benchmark_output(item["final_output"]) for item in items]
...
def collect_metrics(project_root: Path) -> ResultBundle:
    rows: list[dict[str, Any]] = []
    utterance_rows: list[dict[str, Any]] = []
    input_root = project_root / "文档" / "benchmark_inputs"

    base_results = load_json(input_root / "voiceprivacy_style_results.json")
    ...
    recommended_summary = load_json(input_root / "work_metric_attack/final/recommended/recommended_summary.json")
    ...
    ppg_results = load_json(input_root / "work_ppg_tone/voiceprivacy_ppg_tone_results.json")
...
def add_derived_metrics(metrics: pd.DataFrame, source_target_mean: float) -> pd.DataFrame:
    ...
    metrics["content_preservation"] = 1.0 - metrics["wer_capped_at_1"]
    ...
    metrics["report_effect_index"] = (
        ...
        + 0.20 * metrics["content_preservation"]
    )
    metrics.loc[metrics["method_id"] == "source_baseline", "report_effect_index"] = 0.0
...
def save_privacy_utility_scatter(metrics: pd.DataFrame, output_path: Path) -> None:
    ...
    style_axes(ax, "Privacy vs Content Preservation Cost", "ASR WER (lower is better)")
...
def save_utterance_wer_plot(per_utterance: pd.DataFrame, metrics: pd.DataFrame, output_path: Path) -> None:
    ...
    style_axes(ax, "Per-Utterance ASR WER", "WER (lower is better)")
...
def save_report_takeaway_dashboard(metrics: pd.DataFrame, output_path: Path) -> None:
    ...
    best_content = anonymized.loc[anonymized["content_preservation"].idxmax()]
    ...
    draw_metric_tile(
        ...
        f"{source['asr_wer']:.3f} -> {best_content['asr_wer']:.3f}",
        f"Lowest anonymized WER: {best_content['display_name']} (lower is better)",
        "#2563eb",
    )
    draw_metric_tile(
        ax,
        ...
        f"{best_effect['display_name']} balances privacy, identity removal, and content preservation",
        "#7c3aed",
    )
...
def save_method_scoreboard(metrics: pd.DataFrame, output_path: Path) -> None:
    ...
    columns = [
        ...
        ("Content kept", "content_preservation", "1 - WER"),
...
def selected_output_for_source(method_id: str, source_name: str) -> Path | None:
    ...
    for item in items:
        if item.get("source_name") == source_name:
            return resolve_benchmark_output(item["final_output"])
    return None
...
def save_green_waveform_story(project_root: Path, target_gender: str, output_path: Path) -> None:
    ...
    source_waveform = project_root / "文档/展示音频/green_m4a_demo/01_original_denoised.wav"
    paths: list[tuple[str, str, Path]] = [("source_baseline", "Original denoised", source_waveform)]
...
def method_audio_groups(project_root: Path, target_gender: str = "all") -> dict[str, list[Path]]:
    groups: dict[str, list[Path]] = {
        "Original source": [project_root / "文档/音频/test.wav", project_root / "文档/音频/绿色.m4a"],
    }
    method_ids = [
        "female_leaning",
        "male_leaning",
...
def write_markdown_summary(bundle: ResultBundle, output_dir: Path, target_gender: str = "all") -> None:
    ...
    best_eer_tie = anonymized_metrics[anonymized_metrics["asv_eer"] == best_eer_value].sort_values(
        ["identity_privacy_index", "source_similarity_reduction", "content_preservation"],
        ascending=False,
    )
    ...
    report_figure_descriptions = [
        ...
        ("method_scoreboard.png", "compact method comparison across privacy, identity removal, content preservation, and the local effect index."),
    ...
    audit_figure_descriptions = [
        ...
        ("asr_wer_bar.png", "ASR WER content-preservation cost; lower is better."),
        ...
        ("per_utterance_wer_bar.png", "utterance-level ASR WER evidence; lower is better."),
    ]
...
3. **Content preservation is checked by ASR WER:** WER should be as low as possible after anonymization. For the privacy/naturalness trade-off, compare `{tradeoff_text}`.
4. **Best visual effect index:** `{best_effect["display_name"]}` has the highest local effect index, combining ASV EER, source-similarity reduction, and content preservation. This is an explanatory index for the report, not an official VoicePrivacy metric.
...
def main() -> None:
    ...
    save_barplot(metrics, "asr_wer", "ASR WER Content Cost", "ASR WER (lower is better)", output_dir / "asr_wer_bar.png")
    save_barplot(
        metrics,
        "source_similarity_reduction",
